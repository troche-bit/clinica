...
from .services import calcular_item, calcular_totales


class VentaFactCabViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = VentaFactCab.objects.filter(is_deleted=False).select_related(
            'persona', 'timbrado'
        )
        # Filtros opcionales
    ...
    def create(self, request, *args, **kwargs):
        ...
        # ── Contado: cobranza + movimientos de caja ──────────────────────────
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        ser.is_valid(raise_exception=True)
        ser.save(id_usu_modificator=request.user)
    ...
    def perform_destroy(self, instance):
        ...
        now = timezone.now()
        # Borrar lógicamente los registros relacionados
    ...
    def validar_timbrado(self, request):
        ...
        expedicoin = request.data.get('expedicion', '').strip()
        numero   = request.data.get('numero')

        if not all([estab, expedicoin, numero is not None]):
        ...
        if not qs.exists():
            return Response({
                'valido': False,
                'mensaje': f'No hay timbrado vigente para {estab}-{expedicoin}.',
                'timbrado_id': None,
            })
    ...
    def siguiente_numero(self, request):
        ...
        expedicoin = request.query_params.get('expedicion', '').strip()

        if not estab or not expedicoin:
    ...
    def pdf(self, request, pk=None):
        ...
        # Calcular precio unitario por ítem
        for item in detalle:
            try:
        ...
        # Filas vacías para completar mínimo 8 filas
        MIN_FILAS = 8
