"""
Cálculos de IVA según normativa SET Paraguay.
Siempre usar Decimal — nunca float.
"""
...
def calcular_item(monto: Decimal, impuesto: str) -> dict:
    """
    Descompone el monto total de una línea en sus componentes de IVA.

    IVA 10%: gravada = monto / 1.10  →  iva = monto - gravada
    IVA 5%:  gravada = monto / 1.05  →  iva = monto - gravada
    Exenta:  sin gravada ni IVA
    """
...
def calcular_totales(items_calculados: list) -> dict:
    """
    Suma los campos de todos los ítems para construir los totales de cabecera.
    items_calculados: lista de dicts devueltos por calcular_item().
    """
    grav_5  = ZERO
    grav_10 = ZERO
    iva_5   = ZERO
