...
class DocumentoDigPrestador(BaseModel):

    persona_rrhh = models.ForeignKey(
        'persona_rrhh.PersonaRRHH',
        on_delete=models.PROTECT,
        related_name='documentos',
        help_text='Prestador al que pertenece este documento',
    )
    tipo_doc_dig = models.ForeignKey(
        TipoDocDigital,
        on_delete=models.PROTECT,
        related_name='documentos_prestador',
        help_text='Tipo de documento digitalizado',
    )
    storage  = models.TextField(
        help_text='Ruta relativa desde /media/',
    ...
    def __str__(self):
        return f'{self.tipo_doc_dig} — {self.persona_rrhh} ({self.filename})'
