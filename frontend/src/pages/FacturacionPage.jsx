...
import { useCuentasMcb } from '../hooks/useCuentasMcb'
...
function TabCabecera({ form, setForm, detalle, setDetalle, errores }) {
  ...
  // Auto-validar cuando estab + expedicoin + nro de 7 dígitos están completos
  useEffect(() => {
  ...
  return (
    <div className="fac-tab-body">
      ...
      <div className="fac-form-row" style={{ alignItems: 'flex-end', gap: 20 }}>
        ...
        <div className="fac-form-group" style={{ paddingBottom: 6 }}>
          ...
          <span className="fac-cond-hint">
            {form.contado
              ? 'Cobranza en la siguiente pestaña'
              : 'Cuotas en la siguiente pestaña'}
          </span>
        </div>
      </div>
      ...
      {/* Comprobante: establecimiento - expedición - número */}
      <div className="fac-form-group">
        ...
        {/* Timbrado read-only — aparece cuando sigData carga (punto válido con timbrado vigente) */}
        ...
        {/* Resultado validación del número ingresado */}
...
function ModalVerFactura({ id, onClose, onEliminar, showToast, initialModo = 'ver' }) {
  ...
  const [modo, setModo] = useState(initialModo)   // 'ver' | 'editar'
  const [editForm, setEditForm] = useState({ fecha: '', persona: null, observacion: '' })

  // Inicializar formulario cuando llega el dato
  useEffect(() => {
    if (fac) {
      setEditForm({
        fecha:       fac.fecha,
  ...
  if (isLoading) {
    return <div className="fac-modal-loading">Cargando factura…</div>
  }
  if (!fac) return null
    ...
    <div className="fac-ver-wrap">
      ...
      <div className="fac-ver-toolbar">
        {modo === 'ver' ? (
          ...
          <>
            <button
              className="fac-ver-btn save"
              onClick={handleGuardar}
              disabled={actualizar.isPending}
            >
              <Check size={13} /> {actualizar.isPending ? 'Guardando…' : 'Guardar cambios'}
            </button>
            <button className="fac-ver-btn cancel" onClick={() => setModo('ver')}>
              Cancelar
            </button>
          </>
        )}
      ...
      <div className="fac-ver-section">
        <div className="fac-ver-grid-3">
          <div className="fac-ver-field">
            ...
            <span className="fac-ver-val fac-mono" style={{ fontWeight: 700 }}>
          ...
          <div className="fac-ver-field">
            ...
            {modo === 'editar' ? (
              <input
                type="date"
                className="fac-input fac-input-date"
          ...
          {modo === 'editar' ? (
            <div style={{ maxWidth: 420 }}>
              <BuscadorPersona
                ...
                onChange={p => setEditForm(f => ({ ...f, persona: p }))}
              />
              {editForm.persona && (
            ...
            <span className="fac-ver-val">
              {fac.cliente_documento} — {fac.cliente_nombre}
            </span>
          )}
        </div>
...
function ModalFactura({ onClose, onCreado }) {
  ...
  // Al cambiar condición volver a tab 0 si la pestaña activa ya no existe
  ...
  // Retorna el objeto de errores (no un booleano) para usarlo directamente
  const validar = () => {
    const e = {}
  ...
  const handleEmitir = async () => {
    ...
    if (Object.keys(e).length > 0) {
      // Navegar a la pestaña con error usando el objeto recién calculado
    ...
    try {
      ...
      setErrores({ _general: extraerError(err) })
    } finally {
      setGuardando(false)
    }
  }
...
export default function FacturacionPage() {
  ...
  const [facturaViendo, setFacturaViendo] = useState(null)  // { id, modo: 'ver'|'editar' } | null
  ...
  const handleEliminar = async (fac) => {
    if (!window.confirm(`¿Eliminar la factura ${fac.nro_comprobante_formateado}?`)) return
    try {
      await eliminarFactura.mutateAsync(fac.id)
  ...
  return (
    <>
      <style>{`
        ...
        .fac-row-btn { width: 28px; height: 28px; border-radius: 6px; border: 1px solid #fecaca; background: #fff; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #dc2626; }
        .fac-row-btn:hover { background: #fef2f2; }
        ...
        .fac-hint { display: flex; align-items: center; gap: 4px; font-size: 11.5px; color: #6b7280; white-space: nowrap; }
              ...
              <table className="fac-tabla">
                <thead>
                  <tr>
                    ...
                    <th className="fac-th" style={{ width: 48 }}></th>
                  </tr>
                </thead>
                <tbody>
                  {facturas.map(f => (
                    ...
                    >
                      ...
                      <td className="fac-td">
                        <div className="fac-td-acciones" onClick={e => e.stopPropagation()}>
                          <button
                            className="fac-row-btn fac-row-btn-edit"
                            title="Editar"
                          ...
                          <button
                            className="fac-row-btn fac-row-btn-print"
      ...
      {facturaViendo && (
        <Modal
          isOpen={!!facturaViendo}
          onClose={() => setFacturaViendo(null)}
          title="Detalle de factura"
          subtitle="Vista y edición del comprobante"
          size="xl"
        >
          <ModalVerFactura
            id={facturaViendo.id}
            ...
            onClose={() => setFacturaViendo(null)}
            onEliminar={(fac) => handleEliminar(fac)}
            showToast={showToast}
          />
        </Modal>
      )}
