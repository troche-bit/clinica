...
// storage_key es soloLectura en modo editar: cambiarla rompe rutas de archivos ya almacenados.
const CAMPOS_TIPO_DOC = [
  ...
  },
  {
    ...
    requerido:   true,
    soloLectura: true, // no editable después de la creación
  },
]
...
export default function TipoDocDigPage() {
  ...
  const { crear, actualizar, eliminar } = useTipoDocDigMutations()
  ...
  const handleEliminar = (id) => {
    if (window.confirm('¿Eliminar este tipo de documento?')) {
      eliminar.mutate(id, {
        onSuccess: () => showToast('Tipo de documento eliminado.', 'success'),
  ...
  return (
    <>
      <style>{`
        /* ── TipoDocDigPage — layout, encabezado y tabla ── */
