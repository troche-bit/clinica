...
import { usePacientesConDocumentos, useDocumentosPorPaciente, useDeleteDocumento } from '../../hooks/mantenimiento/useDocumentos'

const EXTENSIONES_IMAGEN = ['jpg', 'jpeg', 'png', 'gif', 'webp']

function iconoArchivo(filename) {
  const ext = (filename || '').split('.').pop().toLowerCase()
  if (EXTENSIONES_IMAGEN.includes(ext)) return '🖼️'
  if (ext === 'pdf') return '📄'
  return '📎'
}
...
export default function DocumentosPage() {
  ...
  const cerrarImagenPreview = useCallback(() => {
    if (imagenPreview) URL.revokeObjectURL(imagenPreview.url)
    setImagenPreview(null)
  }, [imagenPreview])

  const handleEliminarConfirmado = useCallback(async () => {
    try {
      await deleteDocumento.mutateAsync(confirmDoc.id)
      showToast('Documento eliminado correctamente.', 'success')
      setConfirmDoc(null)
    } catch (e) {
      showToast(extraerMensajeError(e), 'error')
    }
  }, [confirmDoc, deleteDocumento, showToast])
  ...
  return (
    <>
      <style>{`
        ...
        .dd-pac-item.active { background: #eff6ff; border-color: #bfdbfe; }
        .dd-pac-avatar {
        ...
        .dd-detalle-pac { display: flex; align-items: center; gap: 10px; }
        .dd-detalle-avatar {
        ...
        .dd-tabla { width: 100%; border-collapse: collapse; }
        .dd-th {
        ...
        .dd-archivo-cell { display: flex; align-items: center; gap: 8px; }
        ...
        .dd-img-header {
          ...
          padding: 10px 14px; border-bottom: 1px solid #e8edf2;
        ...
        .dd-img-close:hover { color: #111827; }
        .dd-img-preview {
      ...
      {imagenPreview && (
        <div className="dd-img-overlay" onClick={cerrarImagenPreview}>
          <div className="dd-img-modal" onClick={e => e.stopPropagation()}>
            <div className="dd-img-header">
              <span>{imagenPreview.filename}</span>
              <button className="dd-img-close" onClick={cerrarImagenPreview}>
                <X size={16} />
              </button>
            </div>
            <img
      ...
      <ConfirmDialog
        isOpen={!!confirmDoc}
        title="Eliminar documento"
        description={`¿Eliminar "${confirmDoc?.filename}"? El archivo también se eliminará del servidor y no se podrá recuperar.`}
        onConfirm={handleEliminarConfirmado}
        onCancel={() => setConfirmDoc(null)}
        loading={deleteDocumento.isPending}
      />
