...
import { Plus, Search, Trash2, Eye, X, ChevronDown, AlertCircle } from 'lucide-react'
...
import { useBuscarPersonas, useFormaPago } from '../hooks/useFacturacion'
...
function ModalNuevaCobranza({ onClose, onCreado, showToast }) {
  ...
  useEffect(() => {
    apiClient.get('/cuentas-mcb/').then(r => {
      const lista = r.data?.results ?? r.data ?? []
      setCuentas(lista)
    }).catch(() => {})
  }, [])

  // Al cambiar persona, limpiar selecciones
  ...
  const totalCobrar = cuotasSeleccionadas.reduce((acc, c) => {
    return acc + parseFloat(detalle[c.id]?.monto || '0')
  }, 0)
...
// ── Modal Ver Cobranza ────────────────────────────────────────────────────────
function ModalVerCobranza({ id, onClose, onEliminar, showToast }) {
...
export default function CobranzasPage() {
  ...
  const [cobranzaViendo, setCobranzaViendo] = useState(null) // id
  ...
  return (
    <>
      <style>{`
        ...
        .cob-row-btn { display: flex; align-items: center; justify-content: center; width: 26px; height: 26px; border-radius: 5px; border: 1px solid #e5e7eb; background: #fff; cursor: pointer; transition: all .12s; color: #6b7280; }
        ...
        /* Confirmación */
        .cob-confirm { background: #fff; border-radius: 12px; padding: 28px; text-align: center; display: flex; flex-direction: column; gap: 16px; align-items: center; }
        .cob-confirm-title { font-size: 16px; font-weight: 600; color: #111827; }
        .cob-confirm-sub { font-size: 13px; color: #6b7280; }
        .cob-confirm-btns { display: flex; gap: 10px; }
      `}</style>
      ...
      {confirmando && (
        <Modal
          ...
          size="sm"
        >
          <div className="cob-confirm">
            <AlertCircle size={40} color="#dc2626" />
            <div className="cob-confirm-title">¿Eliminar esta cobranza?</div>
            <div className="cob-confirm-sub">
              Comprobante #{String(confirmando.comprobante_nro).padStart(7, '0')} — {confirmando.cliente_nombre}
            </div>
            <div className="cob-confirm-btns">
