...
import { useCuentasMcb, useCreateCuenta, useUpdateCuenta, useDeleteCuenta } from '../hooks/useCuentasMcb'
...
function PanelMovimiento({ modo, item, ctaActual, onGuardar, onCancelar, guardando }) {
  ...
  return (
    <div className="cta-panel-body">
      ...
      <div className="cta-nota-info">
        Los campos de Factura, Cobranza y Pagos se vincularán automáticamente al implementar esos módulos.
      </div>
...
// ── Vista 2 — Drill-down movimientos ─────────────────────────────────────────
...
function VistaMovimientos({ cuenta, onVolver, onEditarCuenta, onEliminarCuenta }) {
  ...
  const handleEliminar = async (mov) => {
    ...
    try {
      await eliminarMov.mutateAsync(mov.id)
      showToast('Movimiento eliminado.', 'success')
      if (seleccionado?.id === mov.id) cerrarPanel()
    } catch {
...
export default function CuentasMcbPage() {
  ...
  const [panelCuenta, setPanelCuenta] = useState(null) // null | 'crear' | 'editar'
  const [guardando,  setGuardando]  = useState(false)
  ...
  const handleEliminarCuenta = async () => {
    if (!ctaActual) return
    if (!window.confirm(`¿Eliminar la cuenta "${ctaActual.descripcion}"?`)) return
    try {
