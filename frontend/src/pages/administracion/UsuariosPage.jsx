...
const FORM_INIT = {
  username: '', password: '', confirmarPass: '', first_name: '', last_name: '',
  email: '', rol: 'recepcionista', persona_rrhh: '', medicos_asignados: [],
}
...
function ModalUsuario({ usuario, onClose, onSaved }) {
  ...
  const [form, setForm] = useState(isEdit ? {
    ...
    medicos_asignados: usuario.medicos_asignados_ids || [],
  } : { ...FORM_INIT })
  ...
  const toggleMedico = (id) => {
    setForm(f => {
      const ya = f.medicos_asignados.includes(id)
      return {
        ...f,
        medicos_asignados: ya
          ? f.medicos_asignados.filter(m => m !== id)
          : [...f.medicos_asignados, id],
      }
    })
  }
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (payload.rol !== 'secretaria_medico') payload.medicos_asignados = []
      if (isEdit) {
    ...
    <div className="usu-overlay">
      <div className="usu-modal">
        ...
        <div className="usu-modal-body">
          {esMaster && (
            <div className="usu-info-box">
              Usuario master — no se puede cambiar el rol.
            </div>
          )}
          ...
          <div className="usu-grid">
                ...
                <div className="form-group usu-full">
                  ...
                  <div style={{ position: 'relative' }}>
                    <input
                      ...
                      onChange={e => set('confirmarPass', e.target.value)}
            ...
            {form.rol === 'secretaria_medico' && (
              <div className="form-group usu-full">
                ...
                <div className="usu-medicos-list">
                  {prestadores.map(p => (
                    <label key={p.id} className="usu-medico-item">
                      <input
                        type="checkbox"
                        checked={form.medicos_asignados.includes(p.id)}
                        onChange={() => toggleMedico(p.id)}
                      />
                      <span>{p.nombre}</span>
                    </label>
                  ))}
                </div>
                {form.medicos_asignados.length === 0 && (
...
function ModalResetPassword({ usuario, onClose, onSaved }) {
  ...
  return (
    <div className="usu-overlay">
      <div className="usu-modal" style={{ maxWidth: 420 }}>
        <div className="usu-modal-header">
          <div className="usu-modal-title">
  ...
  return (
    <>
      <style>{`
        ...
        .usu-medicos-list { display: flex; flex-direction: column; gap: 6px; max-height: 160px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 10px 12px; background: #fafafa; }
        .usu-medico-item { display: flex; align-items: center; gap: 8px; font-size: 13px; color: #374151; cursor: pointer; font-family: 'DM Sans', sans-serif; }
        .usu-medico-item input { accent-color: #1a3a5c; cursor: pointer; }

        .usu-inactivo td { opacity: .55; }
        .usu-master-badge { display: inline-flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 600; color: #92400e; background: #fef3c7; border: 1px solid #fcd34d; border-radius: 4px; padding: 2px 6px; margin-left: 6px; }
      `}</style>
              ...
              <tbody>
                {usuarios.map(u => (
                  <tr key={u.id} className={!u.activo ? 'usu-inactivo' : ''}>
                    ...
                    <td style={{ color: u.medicos_asignados_nombres?.length ? '#374151' : '#d1d5db' }}>
                      {u.rol === 'secretaria_medico'
                        ? (u.medicos_asignados_nombres?.length
                            ? u.medicos_asignados_nombres.join(', ')
