...
import { useAgendaDia, useAgendaDiaGlobal } from '../hooks/useAgenda'
import {
...
import { useDocumentosPorConsulta, useSubirDocumento, useDeleteDocumento } from '../hooks/useDocumentos'
...
/** Devuelve 'YYYY-MM-DD' en hora local (evita el desfase UTC que muestra el día siguiente). */
function fechaLocal() {
  const d = new Date()
...
// ─── Componente Ticker (tiempo transcurrido) ──────────────────────────────────
...
function PanelDocumentos({ consultaId, pacienteId, tiposDocDig, showToast }) {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      const msg = errData?.archivo?.[0] || errData?.non_field_errors?.[0]
          ...
          {documentos.map(doc => (
            <div key={doc.id} className="cs-doc-item">
              ...
              <div className="cs-doc-actions">
                ...
                <button className="cs-doc-btn cs-doc-btn-danger" onClick={() => handleEliminar(doc.id)} title="Eliminar">
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
...
function PanelConsultaActiva({ consulta, onFinalizar, tiposDocDig, eventosClinicos, showToast }) {
  ...
  // Sincronizar form cuando cambia la consulta seleccionada
  useEffect(() => {
    setForm({
      evento_clinico:  consulta?.evento_clinico  || '',
      proxima_cita:    consulta?.proxima_cita    || '',
  ...
  // Autoguardado silencioso cada 30s
  useEffect(() => {
  ...
  const handleFinalizar = async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.estado?.[0] || 'Error al finalizar la consulta.'
      showToast(msg, 'error')
    } finally {
  ...
  return (
    <div className="cs-panel-activa">
      {/* Dos columnas: datos + edición */}
      <div className="cs-panel-activa-cols">

        {/* Columna izquierda: datos del paciente + historial */}
...
function VistaMedico({ eventosClinicos, tiposDocDig, showToast }) {
  ...
  // Advertir al cerrar/recargar el browser si hay consulta activa
  useEffect(() => {
  ...
  // Cada turno ocupado unificado con su consulta (si existe)
  ...
  // Bloquea iniciar si ya hay una consulta en curso para este médico
  ...
  // Turno sin consulta → crear + iniciar en un paso
  const handleCrearEIniciar = async (turno) => {
    try {
      ...
      const msg = e?.response?.data?.agenda?.[0] ?? e?.response?.data?.estado?.[0] ?? 'Error al iniciar.'
      showToast(msg, 'error')
    }
  }

  // Consulta en_espera → iniciar directamente
  const handleIniciar = async (consulta) => {
    try {
      ...
      const msg = e?.response?.data?.estado?.[0] ?? 'Error al iniciar la consulta.'
      showToast(msg, 'error')
    }
  }
  ...
  return (
    <div className="cs-vista-medico">
      ...
      {/* Banner de advertencia mientras hay consulta activa */}
      {consultaActiva && (
...
export default function ConsultasPage() {
  ...
  // Tipos de documento — se leen desde la API
  const [tiposDocDig, setTiposDocDig] = useState([])
  useEffect(() => {
    apiClient.get('/tipo-doc-dig/', { params: { page_size: 100 } })
      .then(r => setTiposDocDig(r.data?.results || r.data || []))
      .catch(() => {})
  }, [])
  ...
  return (
    <>
      <style>{`
        ...
        /* ── Botón bloqueado (consulta en curso) ── */
        .cs-btn-bloqueado {
