    ...
    <div className="grp-panel-body">
      ...
      <div className="grp-form-group">
        ...
        <div className="grp-toggle-wrap">
          <button
            type="button"
            className={`grp-toggle-btn ${form.activo ? 'active' : ''}`}
      ...
      <div className="grp-panel-acciones">
        <button className="grp-btn-secundario" onClick={onCancelar} disabled={guardando}>
          Cancelar
        </button>
...
// ── Vista 2 — Drill-down productos ────────────────────────────────────────────

function VistaProductos({ grupo, onVolver, onEditarGrupo, onEliminarGrupo }) {
  const [search,       setSearch]       = useState('')
  const [searchInput,  setSearchInput]  = useState('')
  const [panelModo,    setPanelModo]    = useState(null) // null | 'crear' | 'editar'
  ...
  const handleEliminar = async (prod) => {
    ...
    try {
      await eliminarProd.mutateAsync(prod.id)
      showToast('Producto eliminado.', 'success')
      if (seleccionado?.id === prod.id) cerrarPanel()
    } catch {
              ...
              <table className="grp-tabla">
                ...
                <tbody>
                  {productos.map(p => (
                    ...
                    >
                      <td className="grp-td">
                        <span className="grp-prod-nombre">{p.descripcion}</span>
                      </td>
          ...
          {panelModo && (
            <div className="grp-panel">
              <div className="grp-panel-header">
                ...
                <button className="grp-panel-cerrar" onClick={cerrarPanel}>
                  <X size={16} />
                </button>
              </div>
              <PanelProducto
...
export default function GruposPage() {
  const [grupoActual,  setGrupoActual]  = useState(null)  // null = vista grupos
  const [panelGrupo,   setPanelGrupo]   = useState(null)  // null | 'crear' | 'editar'
  const [guardando,    setGuardando]    = useState(false)
  ...
  const handleGuardarGrupo = async (form) => {
    ...
    try {
      if (panelGrupo === 'crear') {
        ...
        const actualizado = await actualizarGrupo.mutateAsync({ id: grupoActual.id, ...form })
        // Actualizar el grupo en contexto con los datos frescos del servidor
  ...
  const handleEliminarGrupo = async () => {
    ...
    if (!window.confirm(`¿Eliminar el grupo "${grupoActual.descripcion}"? Esta acción no se puede deshacer.`)) return
    try {
  ...
  return (
    <>
      <style>{`
        ...
        .grp-btn-icon-edit {
          border-color: #bfdbfe; background: #fff; color: #1a3a5c;
        }
        ...
        .grp-loading {
          text-align: center; color: #9ca3af; padding: 48px; font-size: 13px;
        }

        /* Panel grupo en overlay (sobre vista grupos) */
        .grp-panel-overlay {
      ...
      {/* Panel grupo — overlay flotante (aparece tanto en vista 1 como 2) */}
      {panelGrupo && (
